...
if ($tag_filter > 0) {
    ...
    // Ensure product_tags table is joined only once if multiple tag-related filters were to be added
...
function get_redirect_url_with_filters($base_page = 'shop.php') {
...
if (isset($_POST['verify_age'])) {
     if (isset($_POST['confirm_age']) && $_POST['confirm_age'] === 'yes') {
        ...
        $conn->query("INSERT INTO age_verification_attempts (user_id, ip_address, attempt_successful, details, attempted_at) VALUES ($user_id_to_log, '$ip', 1, 'User confirmed age via popup (shop.php)', NOW())");
        header('Location: shop.php'); 
        exit;
    } else {
        $ip = $_SERVER['REMOTE_ADDR'];
        $user_id_to_log = isset($_SESSION['user_id']) ? $_SESSION['user_id'] : "NULL";
        $conn->query("INSERT INTO age_verification_attempts (user_id, ip_address, attempt_successful, details, attempted_at) VALUES ($user_id_to_log, '$ip', 0, 'User did not confirm age via popup or exited (shop.php)', NOW())");
...
function build_filter_link($params_to_add_or_update) {
    global $current_query_params; // Use the snapshot of $_GET taken earlier
    $merged_params = array_merge($current_query_params, $params_to_add_or_update);
    // For 'clear' type links or setting a filter to 'all' (e.g. category=0 or null)
    foreach ($params_to_add_or_update as $key => $value) {
        if ($value === null || $value === '' || (is_numeric($value) && $value == 0) ) { // 0 for category/tag often means all
            unset($merged_params[$key]);
        }
    }
     // Clean up any other empty params that might have snuck in
    $merged_params = array_filter($merged_params, function($value) { return $value !== null && $value !== ''; });

    return 'shop.php?' . http_build_query($merged_params);
}
...
<head>
    ...
    <title>Shop - 710 Den Glass</title>
    ...
    <div class="age-verification-overlay" id="ageVerificationModal">
        <div class="age-verification-modal">
            <div class="age-verification-content">
                ...
                <form method="post" action="shop.php" class="age-verification-form">
                    <div class="form-check mb-3">
                        <input class="form-check-input" type="checkbox" id="confirmAgeModal" name="confirm_age" value="yes" required>
                        <label class="form-check-label" for="confirmAgeModal">I confirm that I am 21 years of age or older</label>
                    </div>
            ...
            <div class="d-flex align-items-center">
                ...
                <div class="dropdown">
                    ...
                    <ul class="dropdown-menu dropdown-menu-end">
                        ...
                        <li><hr class="dropdown-divider"></li>
                        <li><a class="dropdown-item" href="shop.php?logout=1"><i class="bi bi-box-arrow-right me-2"></i> Logout</a></li>
                    </ul>
                </div>
                <?php else: ?>
            ...
            <div class="col-lg-9">
                <div class="filter-header">
                    ...
                    <p class="active-filters-text">
                        ...
                        if (!empty($search_term) && count($active_filter_parts) == 0 && empty($category_name) && empty($tag_name) ) { /* Already shown in H2 */ }
                        elseif(!empty($search_term)) { /* Display search term in H2 only if it's the sole filter */ }
                     ...
                     <?php if (!empty($category_filter) || !empty($tag_filter) || !empty($search_term) || $min_price !== null || $max_price !== null): ?>
                        <a href="shop.php" class="btn btn-outline-secondary btn-sm btn-clear-filters">
                    ...
                    <div class="row row-cols-1 row-cols-md-2 row-cols-lg-3 g-4">
                        ...
                        <div class="col">
                            <div class="product-card">
                                ...
                                <?php elseif ($product['featured'] == 1 && empty($category_filter) && empty($tag_filter) && empty($search_term) && $min_price === null && $max_price === null): // Show featured badge only on default view ?>
        ...
        <div class="container">
            <div class="row">
                <div class="col-md-4">
                    ...
                    <p>Premium glass pieces for discerning collectors. Quality craftsmanship and exceptional customer service.</p>
                </div>
                <div class="col-md-3">
                    <h4>Quick Links</h4>
